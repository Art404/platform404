...
export default {
  ...
  module: {
    loaders: [
      {
        test: /\.css$/,
        loader: ExtractTextPlugin.extract('style', `css?modules&localIdentName=[name]__[local]${DEBUG ? '' : '-[hash:base64:4]'}`)
      },
      {
        test: /\.scss$/,
        loader: ExtractTextPlugin.extract('style', `css!sass?modules&localIdentName=[name]__[local]${DEBUG ? '' : '-[hash:base64:4]'}`)
      },
      {
        test: /\.js$/,
        loader: 'babel',
